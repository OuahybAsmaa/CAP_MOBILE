...
/// Couleur montant stock v2 — 0 bleu, >0 vert foncé, <0 rouge.
...
/// Source de données stock — magasin physique ou disponibilité web.
...
class DetailProduitPage2 extends ConsumerStatefulWidget {
  ...
  /// Route fade-in vers la fiche détail produit (Infos Stocks, scan…).
  static Route<void> fadeRoute() => PageRouteBuilder<void>(
    pageBuilder: (_, _, _) => const DetailProduitPage2(),
...
/// État page produit v2 — scan DataWedge, onglets nav, colis dépôt, magasin sélectionné.
class _DetailProduitPage2State extends ConsumerState<DetailProduitPage2> {
  ...
  String get _selectedStoreLabel => storeLabelFor(_selectedCodeMag, _stores);
  ...
  Future<void> _pickStore() async {
    ...
    final code =
        ref.read(swappProductProvider).product?.reference ??
        SwappApiConstants.defaultCodeModele;
    await ref
        .read(swappProductProvider.notifier)
        .fetchModele(codeModele: code, codeMag: picked);
  }
  ...
  Future<void> _openColisPopup(ProductStockView product) async {
    final pending = _pendingColisChips(product);
    if (pending.isEmpty || !mounted) return;
    final layout = _Layout2(context);
  ...
  Future<void> _openArticleDialog() async {
    ...
    final current =
        ref.read(swappProductProvider).product?.reference ??
        SwappApiConstants.defaultCodeModele;
    final code = await showArticleCodeDialog(context, initialCode: current);
    if (!mounted || code == null || code.isEmpty) return;
    await ref
        .read(swappProductProvider.notifier)
        .fetchModele(codeModele: code, codeMag: _selectedCodeMag);
    _fetchStockWeb(code);
  }
  ...
  List<String> _sizes(Map<String, Map<String, int>> stock, ProductStockView p) {
    if (stock.isNotEmpty) {
      return stock.keys.toList()..sort((a, b) {
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        _ProductCard2(
                          ...
                          showColisButton:
                              _navIndex == 0 &&
                              _stockSource == _StockSource.mag,
                          pendingColisCount:
                              (_navIndex == 0 &&
                                  _stockSource == _StockSource.mag)
                              ? _pendingColisChips(product).length
                              : 0,
                          onColisPackTap: () => _openColisPopup(product),
                        ),
...
// Auteur : H.AMIZIANI
// ---------------------------------------------------------------------------
/// En-tête v2 — retour, titre page, langue, clients et panier.
class _Header2 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          _HeaderActionBtn(l: l, icon: Icons.arrow_back, onTap: onBack),
...
/// Bouton circulaire header — retour, langue, clients (icône blanche sur fond semi-transparent).
...
/// Bouton panier header — icône chariot + badge quantité panier (orange).
...
/// Icône carrée colorée — recherche article, NFC, scan QR dans la carte produit.
...
/// Bandeau erreur — message API ou chargement affiché sous le header.
class _ErrorBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        message,
        style: TextStyle(color: AppColors.error, fontSize: l.sp(12)),
      ),
    );
  }
}
...
// Auteur : H.AMIZIANI
// ---------------------------------------------------------------------------
/// Carte hero produit v2 — réassort, photo, pills, bouton colis et sélecteur magasin.
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Expanded(
                child: ReassortChip(ok: product.reassortOk, dp: l.dp, sp: l.sp),
              ),
              SizedBox(width: l.dp(4)),
              Row(
...
/// Pastille hero — modèle (outline) ou prix (filled) ; variante compacte pour la ligne pills.
...
// Auteur : H.AMIZIANI
// ---------------------------------------------------------------------------
/// Grille récap stock — cartes dispo/transit/picking/dépôt (ou dispo web seule).
      ...
      child: Row(
        ...
        children: [
          for (var i = 0; i < items.length; i++) ...[
            ...
            Expanded(
              child: _OverviewCard2(l: l, item: items[i]),
            ),
          ],
        ],
      ),
...
/// Données carte overview — libellé, valeur totale, icône et couleur thématique.
class _OverviewItem {
  final String label;
  final int value;
  final IconData icon;
...
/// Carte overview — une métrique stock (dispo, transit, picking, dépôt) avec icône et total.
...
/// Placeholder onglet — message « bientôt disponible » pour les sections non implémentées.
...
// Auteur : H.AMIZIANI
// ---------------------------------------------------------------------------
/// Ligne tableau stock — bandeau taille coloré + cellules métriques par colonne.
                ...
                child: Row(
                  children: [
                    for (final m in metrics)
                      Expanded(
                        child: _MetricCell2(l: l, metric: m),
                      ),
                    Icon(
                      Icons.chevron_right,
                      size: l.dp(18),
...
/// Définition métrique tableau — libellé colonne, valeur, icône et couleur pour _MetricCell2.
class _MetricDef {
  final String label;
  final int value;
  final IconData icon;
...
/// Cellule métrique — icône, libellé, valeur colorée et barre indicatrice dans une ligne taille.
...
// Auteur : H.AMIZIANI
// ---------------------------------------------------------------------------
/// Barre navigation bas — 5 onglets (Stock Mag, Web, Alentours, Avis, Réserve).
