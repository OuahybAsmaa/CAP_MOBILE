...
//                  (_ResultsView) ; PopScope gère retour liste→formulaire→parent ;
//                  icône + header → ClientAddPage ; loupe header + bouton Rechercher.
// Points d'entrée : header produit (DetailProduitPage / DetailProduitPage2) et
//                  tuile « Mes clients » du menu Swapp (ClientSearchPage.fadeRoute).
...
import 'package:cap_mobile/swapp/pages/client/client_add_page.dart';
...
class ClientSearchPage extends ConsumerStatefulWidget {
  /// Vide la recherche précédente à l'ouverture pour toujours afficher le
  /// formulaire. Vrai quand la page est un point d'entrée du module (menu),
  /// faux depuis la fiche produit où l'on revient sur ses résultats.
  final bool startFresh;

  const ClientSearchPage({super.key, this.startFresh = false});

  /// Route fade-in vers le module client — utilisée par la tuile « Mes clients ».
  static Route<void> fadeRoute({bool startFresh = true}) =>
      swappMenuFadeRoute(ClientSearchPage(startFresh: startFresh));
...
class _ClientSearchPageState extends ConsumerState<ClientSearchPage> {
  ...
  void initState() {
    super.initState();
    // Aucun widget monté n'écoute encore le provider : reset synchrone sûr.
    if (widget.startFresh) {
      ref.read(clientSearchProvider.notifier).reset();
    }
  }
  ...
  /// UI : Retour formulaire → page appelante, fiche produit ou menu (Navigator.pop).
  void _handleBack() {
  ...
  void _openAddClient() {
    Navigator.push(
      context,
      PageRouteBuilder(
        pageBuilder: (_, _, _) => const ClientAddPage(),
...
class _ResultsView extends StatelessWidget {
  ...
  const _ResultsView({super.key, required this.state, required this.onRetry});
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) =>
          _ClientCard(client: state.results[index]),
    );
  }
}
