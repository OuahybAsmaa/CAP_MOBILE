...
class SwappApiService {
  ...
  Future<List<StockWebItem>> fetchStockWeb({
    required String codeModele,
  }) async {
  ...
  Future<List<NearbyStoreStock>> fetchNearbyStock({
    ...
    final uri = Uri.parse(
      SwappApiConstants.nearbyStockUrl(article, mag),
    );
      ...
      switch (response.statusCode) {
        case 200:
          ...
          return json
              ...
              .entries
              .map(
                (entry) => NearbyStoreStock.fromJson(
                  entry.value,
                  rank: entry.key + 1,
                ),
              )
    ...
    try {
      ...
      switch (response.statusCode) {
        case 200:
          ...
          return json
              .whereType<Map>()
              .map((entry) => ProductReviewItem.fromJson(
