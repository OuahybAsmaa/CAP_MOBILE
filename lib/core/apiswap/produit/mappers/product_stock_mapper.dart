...
class ProductStockMapper {
  ...
  static ProductStockView fromModeleGlobal(ModeleGlobalModel model) {
    ...
    return ProductStockView(
      ...
      colisDepotChips: _buildColisDepotChips(model),
    );
  }

  static List<ColisDepotChip> _buildColisDepotChips(ModeleGlobalModel model) {
    final depotResa = model.colisDepotResa;
    final depot = model.colisDepot;
    final chips = <ColisDepotChip>[
      for (var i = 0; i < depotResa.length; i++)
        ColisDepotChip(
          id: 'resa_$i',
          label: depotResa[i].labelDepotResa(),
          isDepotResa: true,
        ),
      for (var i = 0; i < depot.length; i++)
        ColisDepotChip(
          id: 'depot_$i',
          label: depot[i].labelDepot(),
          isDepotResa: false,
        ),
    ];
    if (chips.isEmpty) return demoColisDepotChips();
    return chips;
  }
