...
import '../apiswap/shared/config/swapp_api_constants.dart';
...
class ClientService {
  ...
  List<ClientItem> _filterMock(ClientSearchQuery query) {
    ...
    if (nom.isNotEmpty) {
      results = results.where((c) => c.lastName.toLowerCase().contains(nom));
    }
    ...
    if (email.isNotEmpty) {
      results = results.where((c) => c.email.toLowerCase().contains(email));
    }
